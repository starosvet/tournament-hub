/* ============================================================
   Tournament Hub — Bracket Controller (FIXED v6)
   ============================================================ */
(function () {
  'use strict';
  ...
  async function vote(matchId, playerNumber) {
    ...
    if (window.TH && typeof window.TH.castVote === 'function') {
      try { await window.TH.castVote(matchId, playerNumber); } catch (e) { console.warn("Server vote failed:", e); }
    }
    window.DB.saveDB(db);
  ...
  function finishMatch(matchId) {
    ...
    for (const t of (db.tournaments || [])) {
      const found = findMatchInTournament(t, matchId);
      if (found) { foundData = { ...found, tournament: t }; break; }
    }
    ...
    if ((match.votes1 || 0) >= (match.votes2 || 0)) match.winner_id = match.player1 ? match.player1.id : null;
    else match.winner_id = match.player2 ? match.player2.id : null;
    match.finished = true;
    ...
    if (allMatchesFinished) window.Tournament.advanceRound(tournament.id);
    else { window.TournamentEngine.propagateWinners(tournament.rounds); window.DB.saveDB(db); }
