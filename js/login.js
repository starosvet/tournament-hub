/* ============================================================
   Tournament Hub — Database Layer (FIXED v7 — clean init, OAuth redirect)
   ============================================================ */
...
(function () {
  ...
  /* ==========================================================
     INIT (FIXED: чистая инициализация, OAuth redirect)
     ========================================================== */
  async function init() {
    if (!window.TH || !USE_SUPABASE) return;

    // 🔧 FIX: Ждём полной загрузки всех скриптов
    await new Promise(resolve => {
      if (document.readyState === 'complete') resolve();
      else window.addEventListener('load', resolve);
    });

    // 🔧 FIX: Подписываемся на auth changes ПЕРВЫМ делом
    window.TH.onAuthStateChange(async (event, session) => {
      ...
      if (event === 'SIGNED_IN' || event === 'TOKEN_REFRESHED') {
        ...
        // 🔧 FIX: Если мы на login.html после OAuth — редиректим на главную
        if (window.location.pathname.includes('login.html')) {
          const basePath = window.TH.getBasePath ? window.TH.getBasePath() : '/tournament-hub';
      ...
      // 🔧 FIX: Рендерим навигацию ПОСЛЕ обработки auth
      if (typeof Auth !== 'undefined' && Auth.renderNavUser) {
        Auth.renderNavUser();
      }
    });

    // 🔧 FIX: Проверяем текущую сессию (обработка OAuth-возврата)
    const session = await window.TH.getSession();
    if (session?.user) {
      ...
      // 🔧 FIX: Если на login.html с активной сессией — редирект
      if (window.location.pathname.includes('login.html')) {
        const basePath = window.TH.getBasePath ? window.TH.getBasePath() : '/tournament-hub';
        window.location.href = basePath + '/index.html';
      }
  ...
  window.addEventListener("storage", function(e) {
    ...
    if (e.key === "th_user") {
      // 🔧 FIX: Убран dispatchEvent
    }
  });

  // 🔧 FIX: Убран слушатель th-db-changed — вызывал циклические ререндеры
