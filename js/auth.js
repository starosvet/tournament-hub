/* ============================================================
   Tournament Hub Authentication (FIXED v7 — closed IIFE, safe init, no leaks)
   ============================================================ */
(function () {
  'use strict';
  ...
  async function register(username, password, email) {
    ...
    if (!email?.includes('@')) return { success: false, error: "Введите валидный адрес email" };
  ...
  async function login(email, password) {
    if (!email || !password) return { success: false, error: "Заполните все поля авторизации" };
  ...
  async function logout() {
    try {
      ...
      console.warn("Supabase auth state clear failed, proceeding with local purge");
    }
  ...
  async function canUserVote(matchId) {
    ...
    if (activeTourney?.config) {
      if (!user && !activeTourney.config.allowGuest) {
        return { can: false, reason: "Голосование гостям в данном турнире запрещено. Пожалуйста, авторизуйтесь." };
      }
    }

    const votedList = JSON.parse(localStorage.getItem("th_voted_matches") || "[]");
    if (votedList.includes(matchId)) {
      return { can: false, reason: "Вы уже отдали свой голос в этом матче!" };
    }
    ...
    if (window.TH && user) {
      ...
      if (votedServer) {
        markVote(matchId);
        return { can: false, reason: "Сервер подтвердил, что ваш голос уже учтен в этой сетке." };
      }
    }
    return { can: true };
  }
  ...
  function renderNavUser() {
    ...
    window.DB.getCurrentUser().then(user => {
      if (user) {
        ...
        box.innerHTML = `
          <div class="user-nav-profile" style="display:flex;align-items:center;gap:12px;">
            <a href="profile.html" style="display:flex;align-items:center;gap:8px;text-decoration:none;color:var(--text);font-weight:500;">
              ...
              <span>${escapeFn(user.displayName || user.username)}</span>
              ${fandomBadge}
              ${roleBadge}
            </a>
        ...
        box.innerHTML = `
          <a href="login.html" class="btn-primary" style="padding:6px 14px;font-size:13px;text-decoration:none;display:inline-block;">🔐 Войти</a>
        `;
      }
    }).catch(e => {
      console.error('Nav user critical render error:', e);
  ...
  function checkFandomAutoAdmin() {
    window.DB.getCurrentUser().then(user => {
      ...
      window.TH.getSiteSettings().then(({ data }) => {
        if (data?.fandom_admins && Array.isArray(data.fandom_admins)) {
  ...
  // Экспорт публичных методов модуля аутентификации
  window.Auth = {
