...
(function () {
    ...
    function renderNavUser() {
        ...
        if (user) {
            box.innerHTML = `
                <a href="profile.html?id=${encodeURIComponent(user.id)}" style="color:var(--text);text-decoration:none;font-weight:600;margin-right:12px;">${escapeHTML(user.displayName || user.username)}</a>
                <button onclick="Auth.logout();location.reload()" style="background:var(--bg-3);border:1px solid var(--border);color:var(--text-2);padding:6px 14px;border-radius:8px;cursor:pointer;font-size:13px;">Выйти</button>
            `;
        } else {
            box.innerHTML = `<a href="login.html" style="color:var(--accent);text-decoration:none;font-weight:600;font-size:14px;">Войти</a>`;
        }
    }
