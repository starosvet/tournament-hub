/* ============================================================
   Tournament Hub Comments system (FIXED v4 — unsubscribe, safe subscribe, no leaks)
   ============================================================ */
...
(function () {
  ...
  function subscribeToComments(tournamentId, container) {
    ...
    if (activeChannels.has(tournamentId)) {
      try {
        ...
        window.TH.getClient().removeChannel(oldChannel);
      } catch (e) {}
    }

    subscribedTournaments.add(tournamentId);

    const channel = window.TH.getClient()
  ...
  function unsubscribeFromComments(tournamentId) {
    if (activeChannels.has(tournamentId)) {
      try {
        const channel = activeChannels.get(tournamentId);
        window.TH.getClient().removeChannel(channel);
      } catch (e) {}
      activeChannels.delete(tournamentId);
      subscribedTournaments.delete(tournamentId);
    }
  }
