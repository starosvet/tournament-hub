/* ============================================================
   Tournament Hub — User Profile & Fandom Dashboard (FIXED v5)
   ============================================================ */
(function () {
  'use strict';
      ...
      if (user.fandomVerified && user.fandomName) {
        fStatus.innerHTML = `
          <div style="background:rgba(0,200,100,0.1); border:1px solid #00c864; padding:12px; border-radius:6px; margin-top:8px;">
            <p style="color:#00c864; margin:0; font-weight:bold;">✓ Аккаунт Fandom привязан успешно</p>
            <p style="margin:4px 0 0 0; font-size:13px;">Никнейм на Вики: <strong>${window.DB.escapeHTML(user.fandomName)}</strong></p>
            <button id="btnUnlinkFandom" class="btn-secondary" style="margin-top:10px; padding:4px 12px; font-size:12px;">Отвязать аккаунт</button>
          </div>
        `;
        ...
        fStatus.innerHTML = `
          <div style="background:var(--bg-2); border:1px solid var(--border); padding:16px; border-radius:6px; margin-top:8px;">
            <p style="margin:0 0 10px 0; font-size:13px; color:var(--text-2);">Свяжите профиль с Fandom, чтобы автоматически получить права модератора, если вы являетесь администратором вики.</p>
            <div style="display:flex; gap:8px;">
              <input type="text" id="inputFandomName" placeholder="Ваш ник на Fandom" style="flex:1; margin:0;">
              <button id="btnLinkFandom" class="btn-primary" style="margin:0; padding:8px 16px;">Привязать</button>
            </div>
          </div>
        `;
  ...
  async function saveProfileChanges() {
    ...
    try {
      const updated = await window.TH.updateProfile({
        display_name: dispVal,
        avatar: avatVal
      });

      const user = await window.DB.getCurrentUser();
      if (user) {
        user.displayName = updated.display_name;
  ...
  async function executeFandomLink() {
    ...
    if (!fName) {
      alert("Укажите ваш точный никнейм на платформе Fandom.");
      return;
    }

    try {
      // Имитируем верификационный запрос к API Fandom (или прямую запись метаданных)
      const updated = await window.TH.updateProfile({
        fandom_name: fName,
      ...
      const user = await window.DB.getCurrentUser();
      if (user) {
        user.fandomName = updated.fandom_name;
        user.fandomVerified = updated.fandom_verified;
        await window.DB.setCurrentUser(user);
      }

      alert(`Аккаунт Fandom (${fName}) успешно верифицирован!`);
      if (window.Auth && window.Auth.checkFandomAutoAdmin) await window.Auth.checkFandomAutoAdmin();
      window.location.reload();
    } catch (e) {
      alert("Не удалось привязать Fandom: " + e.message);
    }
  }

  async function executeFandomUnlink() {
    if (!confirm("Вы уверены, что хотите отвязать ваш Fandom-аккаунт? Вы можете потерять права администратора.")) return;

    const res = await window.Auth.unlinkFandom();
    if (res.success) {
      alert("Fandom аккаунт успешно отвязан.");
      window.location.reload();
    } else {
      alert("Ошибка при удалении привязки: " + res.error);
    }
  }
  ...
  function initProfilePage() {
    window.DB.getCurrentUser().then(user => {
      if (!user) {
        alert("Для просмотра личного кабинета необходимо авторизоваться.");
